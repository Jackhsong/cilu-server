...
<script>
		...
		$.messager.confirm('删除','确定删除吗？',function(b){
			if(b){
				...
				$.ajax({
		            ...
		            type: 'post',
		            dataType: 'json',
		            data: {'id':id, 'isAvailable':0, type:"product"},
	...
	function modifyImage(index){
    	var arr=$("#s_data").datagrid("getData");
    	window.location.href = "${rc.contextPath}/image/edit/"+arr.rows[index].id+"?type=product"
	}
	...
	$(function(){
		...
		$('#s_data').datagrid({
            ...
            toolbar:[
            {
                ...
                iconCls:'icon-add',
                handler:function(){
                	window.location.href = "${rc.contextPath}/image/add?type=product"
                }
            },'-',
            {
                iconCls: 'icon-remove',
                        ...
                        $.messager.confirm('删除','确定删除吗',function(b){
                            if(b){
                                ...
                                $.post("${rc.contextPath}/image/batchDelete", //批量删除
									{ids: ids.join(","),isAvailable:0, type:"product"},
									function(data){
										if(data.status == 1){
