...
$(function(){
	...
	function editIt(index){
		...
		window.location.href = "${rc.contextPath}/categoryBanner/edit/"+arr.rows[index].id
	}
	...
	function updateActivity(row){
		$.ajax({
			url: '${rc.contextPath}/categoryBanner/updateOrder',
			type:"POST",
	...
	function displayIt(id,code){
    	$.messager.confirm("提示信息","确定修改展现状态吗？",function(re){
        	if(re){
            	$.messager.progress();
            	$.post("${rc.contextPath}/categoryBanner/updateDisplayCode",{id:id,code:code},function(data){
	...
	$(function(){
		$('#s_data').datagrid({
            ...
            collapsible:true,
            idField:'id',
            url:'${rc.contextPath}/categoryBanner/jsonBannerWindowInfo',
            loadMsg:'正在装载数据...',
            queryParams:{
            	modelId:'${modelId}'
            },
            ...
            toolbar:[{
                ...
                iconCls:'icon-add',
                handler:function(){
                    window.location.href = "${rc.contextPath}/categoryBanner/addBanner/"+${modelId}
                }
            }],
            pagination:true,
            rownumbers:true
