...
$(function(){
	...
	function editIt(index){
		...
		window.location.href = "${rc.contextPath}/mwebGroupBanner/edit/"+arr.rows[index].id
	}
	...
	function updateActivity(row){
		$.ajax({
			url: '${rc.contextPath}/mwebGroupBanner/updateOrder',
			type:"POST",
	...
	function displayIt(id,code){
    	$.messager.confirm("提示信息","确定修改展现状态吗？",function(re){
        	if(re){
            	$.messager.progress();
            	$.post("${rc.contextPath}/mwebGroupBanner/updateDisplayCode",{id:id,code:code},function(data){
	...
	$(function(){
		$('#s_data').datagrid({
            ...
            collapsible:true,
            idField:'id',
            url:'${rc.contextPath}/mwebGroupBanner/jsonBannerWindowInfo',
            loadMsg:'正在装载数据...',
            fitColumns:true,
            remoteSort: true,
            singleSelect:true,
            ...
            toolbar:[{
                ...
                iconCls:'icon-add',
                handler:function(){
                    window.location.href = "${rc.contextPath}/mwebGroupBanner/addBanner"
                }
            }],
            pagination:true,
            rownumbers:true
