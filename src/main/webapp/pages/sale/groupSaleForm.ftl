...
<div data-options="region:'center',title:'添加商品组合信息'" style="padding:5px;">
	...
	<form id="saveAcCommon" action="${rc.contextPath}/sale/saveGroupSale"  method="post">
		<fieldset>
    		...
    		&nbsp;&nbsp;&nbsp;&nbsp;标题：<input type="text" maxlength="20" id="name" name="name" style="width:300px;"  value="<#if acCommon.name?exists>${acCommon.name}</#if>" /><font color="red">将在组合特卖页面标题位置展示，请认真填写（勿使用"专场"字样）。注：最多输入10个汉字或者20个字母/数字</font><br/><br/>
			&nbsp;&nbsp;&nbsp;描述：<textarea name="gegesay" onkeydown="checkEnter(event)" id="gegesay" style="height: 60px;width: 300px"><#if acCommon.gegesay?exists>${acCommon.gegesay}</#if></textarea>&nbsp;&nbsp;长度（&lt; 140）：<span style="color:red" id="counter">0 字</span><br/><br/>
			&nbsp;&nbsp;&nbsp;&nbsp;备注：<input type="text" maxlength="45" id="desc" name="desc" style="width:300px;"  value="<#if acCommon.desc?exists>${acCommon.desc}</#if>" /><br/><br/>
			 &nbsp;<br/><br/>
            ...
            <a onclick="picDialogOpen('newImage17')" href="javascript:;" class="easyui-linkbutton">上传图片</a><span style="color:red"></span><br/><br/>
	...
	$(function(){
		$("#saveButton").click(function(){
	    	...
	    	if($.trim(title) == ""){
	    		...
	    		$.messager.alert("提示","组合标题字数超过限制","info");
	    	}else if($.trim(gegesay) == ""){
	    		$.messager.alert("提示","描述必填","info");
	    	}else if(gegesay.length>140){
	    		$.messager.alert("提示","描述字数不得超过140","info");
	    	}/* else if($.trim(weixin) == ""){
