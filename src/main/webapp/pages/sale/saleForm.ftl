...
<div data-options="region:'center',title:'添加特卖信息'" style="padding:5px;">
	...
	<form id="saveSaleWindow" action="${rc.contextPath}/indexSale/save"  method="post">
		<fieldset>
			...
			<div id="categoryFirstId_div">
				<span>所属分类:
					<select name="categoryFirstId" class="selectStyle">
						...
						<option value="${first.id?c}"<#if saleWindow.categoryFirstId?exists && (saleWindow.categoryFirstId == first.id)>selected</#if>>${first.name}</option>
						</#list>
					</select>
				 </span><span style="color:red">必须</span>
			 </div>
			 <br/>
			 特卖展现状态:
