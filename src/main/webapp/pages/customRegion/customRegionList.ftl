...
<div data-options="region:'center'" style="padding:5px;">
	<div id="cc" class="easyui-layout" data-options="fit:true" >
		<div data-options="region:'north',title:'自定义版块管理-旧版本',split:true" style="height: 90px;">
			<br/>查看以下版块：
			<input type="checkbox" name="searchIsAvailable" value="1" checked/>可用
		    <input type="checkbox" name="searchIsAvailable" value="0" />停用
			<input type="checkbox" name="searchIsDisplay" value="1" checked/>可见
		    <input type="checkbox" name="searchIsDisplay" value="0" />不可见
		</div>
...
<script>
	function searchCustomRegion(){
		...
		var isDisplay =[]; 
		$("input[name='searchIsDisplay']:checked").each(function(){ 
	...
	function updateActivity(row){
		$.ajax({
			url: '${rc.contextPath}/customRegion/updateCustomRegionSequence',
			type:"POST",
	...
	function displayIt(id,code){
    	...
    	$.messager.confirm("提示信息",tips,function(re){
        	if(re){
            	$.messager.progress();
            	$.post("${rc.contextPath}/customRegion/updateCustomRegionDisplayStatus",{id:id,code:code},function(data){
	...
	function availableIt(id,code){
    	...
    	$.messager.confirm("提示信息",tips,function(re){
        	if(re){
            	$.messager.progress();
            	$.post("${rc.contextPath}/customRegion/updateCustomRegionAvailableStatus",{id:id,code:code},function(data){
                	$.messager.progress('close');
                	if(data.status == 1){
                        $('#s_data').datagrid('reload');
                	} else{
	...
	$(function(){
		...
		$("input[name='searchIsDisplay']").each(function(){
			$(this).change(function(){
				searchCustomRegion();
			});
		});
		...
		$('#s_data').datagrid({
            ...
            collapsible:true,
            idField:'id',
            url:'${rc.contextPath}/customRegion/jsonCustomRegionInfo',
            loadMsg:'正在装载数据...',
            singleSelect:false,
            fitColumns:true,
            remoteSort: true,
                ...
                {field:'hidden',  title:'操作', width:80,align:'center',
                    formatter:function(value,row,index){
                    	if (row.editing){
                            ...
                            var c = '<a target="_blank" href="${rc.contextPath}/customRegion/manageLayout/' + row.id + '">管理布局</a> | ';
                            var d = '';
                            var e = '';
                            if(row.isDisplay == 1){
                    ...
                    if(rows.length > 0){
                        $.messager.confirm('展现','确认全部设为可用吗？',function(b){
                            if(b){
                                ...
                                $.post("${rc.contextPath}/customRegion/updateCustomRegionAvailableStatus",
									{id: ids.join(","),code:1},
									function(data){
										$.messager.progress('close');
										if(data.status == 1){
                    ...
                    if(rows.length > 0){
                        $.messager.confirm('展现','确认全部设为可见吗？',function(b){
                            if(b){
                                ...
                                $.post("${rc.contextPath}/customRegion/updateCustomRegionDisplayStatus",
									{id: ids.join(","),code:1},
									function(data){
										$.messager.progress('close');
										if(data.status == 1){
	    	...
	    	buttons: [
	    	{
	    		...
	    		handler: function(){
	    			$('#editCustomRegionForm').form('submit',{
	    				url: "${rc.contextPath}/customRegion/saveOrUpdateCustonRegion",
	    				onSubmit:function(){
	    					$.messager.progress();
	    				},
	    				success: function(data){
