...
<div data-options="region:'center'" style="padding:5px;">
	<div id="cc" class="easyui-layout" data-options="fit:true" >
		<div data-options="region:'north',title:'分类管理',split:true" style="height: 120px;">
			<br/>
		...
		<div data-options="region:'center'" >
		    ...
		    <div id="editCustomGGRecommendDiv" class="easyui-dialog" style="width:800px;height:700px;padding:15px 20px;">
		        <form id="editCustomGGRecommendForm" method="post">
					...
					<p>
						<span>分类名称：</span>
						<span><input type="text" name="name" id="editCustomGGRecommendForm_title" maxlength="100" style="width: 300px;"/></span>
					...
					<p>
						...
						<span style="color: red">宽:88 高:88&nbsp;</span><span style="color: red" id="oneTip"></span><br>
						<span><img id="oneImg" src=""></span>
					</p>
					<p>
						<span>关联类型：</span>
						<span><input type="radio" name="type" value="1"/>组合&nbsp;&nbsp;</span>
						<span><input type="radio" name="type" value="2"/>网页自定义活动&nbsp;&nbsp;</span>
						<span><input type="radio" name="type" value="3"/>原生自定义活动&nbsp;&nbsp;</span>
						<span><input type="radio" name="type" value="4"/>商城分类&nbsp;&nbsp;</span>
						<span style="color: red">*</span>
					</p>
					<p id="mallClassification">
						<span>商城类目：</span>
						 <tr>
							<td class="searchText"><input id="firstCategoryId" type="text" name="firstCategoryId" /></td>
		                    <td class="searchText"><input id="secondCategoryId" type="text" name="secondCategoryId" /></td>
							<td class="searchText"><input id="thirdCategoryId" type="text" name="thirdCategoryId" /></td>
		                </tr>
						<span style="color: red">*</span>
					</p>
 ...
 return date;
	...
	function valid() {
		...
		if($('input[name=type]:checked').val() == 4) {
			if($('#firstCategoryId').combobox('getValue') == '') {
				$.messager.alert('警告','商城类目第一级分类不能为空', 'warning');
				return false;
			}
			if($('#secondCategoryId').combobox('getValue') == '') {
				$.messager.alert('警告','商城类目第二级分类不能为空', 'warning');
				return false;
			}
			//if($('#thirdCategoryId').combobox('getValue') == '') {
				//$.messager.alert('警告','商城类目第三级分类不能为空', 'warning');
				//return false;
			//}
	...
	function edit(id) {
		...
		$('#display').hide();
		$.ajax({
			url: '${rc.contextPath}/categoryClassification/findById?id=' + id,
			type:"POST",
			async:false,
			success: function(data) {
				...
				if(data.status == 1) {
					...
					$('#oneTip').text('宽度：88px');
					if(data.data.type == 4) {
						$('#mallClassification').show();
						$('#firstCategoryId').combobox('setValue', data.data.firstCategoryId);
						$('#secondCategoryId').combobox('setValue', data.data.secondCategoryId);
						$('#thirdCategoryId').combobox('setValue', data.data.thirdCategoryId);
	...
	function deleteIt(id){
	    $.messager.confirm("提示信息","确定删除么？",function(re){
	        if(re){
	            $.messager.progress();
	            $.post("${rc.contextPath}/categoryClassification/delete",{id:id},function(data){
	                $.messager.progress('close');
	                if(data.status == 1){
	...
	function updateActivity(row){
		$.ajax({
			url: '${rc.contextPath}/categoryClassification/saveOrUpdate',
			type:"POST",
	...
	$(function() {
		...
		$('#firstCategoryId').combobox({   
		    ...
		    onSelect:function(rac){
		    	...
		    	$('#secondCategoryId').combobox('reload',url);
		    }
		...
		$('#secondCategoryId').combobox({
		    ...
		    onSelect:function(rac){
		    	...
		    	$('#thirdCategoryId').combobox('reload',url);
		    }
		});

		$('#thirdCategoryId').combobox({
		...
		$('#s_data').datagrid({
            ...
            collapsible:true,
            idField:'id',
            url:'${rc.contextPath}/categoryClassification/listInfo',
            loadMsg:'正在装载数据...',
            queryParams:{
            ...
            columns:[[
                ...
                {field:'imageURL',    title:'分类图片', width:80, align:'center'},
