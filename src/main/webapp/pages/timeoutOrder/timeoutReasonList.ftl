...
<div title="订单超时原因管理" class="easyui-panel" style="padding:10px">
...
function updateIsAvailable(id,code){
	...
	$.messager.confirm("提示信息",tip,function(ra){
        if(ra){
	            $.messager.progress();
	            $.post(
	            		"${rc.contextPath}/timeoutOrder/updateTimeoutReasonStatus",
	...
	$(function(){
		...
		$('#addTemplate').dialog({
            ...
            buttons:[{
                ...
                handler:function(){
                    $('#af').form('submit',{
                        url:"${rc.contextPath}/timeoutOrder/saveTimeoutReason",
                        onSubmit: function(){ 
		...
		$('#s_data').datagrid({
            ...
            collapsible:true,
            idField:'id',
            url:'${rc.contextPath}/timeoutOrder/jsonTimeoutReasonInfo',
            loadMsg:'正在装载数据...',
            fitColumns:true,
            remoteSort: true,
            singleSelect:true,
