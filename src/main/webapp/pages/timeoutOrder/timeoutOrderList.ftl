...
<body class="easyui-layout">
	...
	<div data-options="region:'center'" style="padding: 5px;">
		<div id="cc" class="easyui-layout" data-options="fit:true" >
			<div data-options="region:'north',title:'条件筛选-订单管理',split:true" style="height: 190px;">
				<form action="${rc.contextPath}/timeoutOrder/exportTimeoutResult" id="searchForm" method="post" >
						<input type="hidden" id="startTime" name="startTime" value="${startTime!''}"/>
						<input type="hidden" id="endTime" name="endTime" value="${endTime!''}"/>
						<input type="hidden" id="exportType" name="exportType" value="${exportType!'1'}"/>
						...
						<table class="search">
							...
							<tr>
                                ...
                                <td class="searchText"><input id="productCode" name="productCode" value="" " /></td>
                                <td class="searchName">收货手机号</td>
                                <td class="searchText"><input id="receivePhone" name="receivePhone" value="" " /></td>
							...
							<tr>
                                ...
                                <td class="searchText">
									...
									<a id="exportResult" onclick="exportResult()" href="javascript:;" class="easyui-linkbutton" data-options="iconCls:'icon-print'">导出</a>&nbsp;
                                </td>
                                <td class="searchName"></td>
	...
	<script>
		...
		function complainDetail(orderId){
			...
			var url = "${rc.contextPath}/timeoutOrder/complainDetail/" + parseInt(orderId);
		...
		function qcDeal(index){
			...
			var arr=$("#s_data").datagrid("getData");
			$('#qcDealForm_orderId').val(arr.rows[index].id);
		...
		$(function() {
			...
			$('#complainDetail_div').dialog({
	            title: '申诉明细',
	            closed: true,
	            href: '${rc.contextPath}/timeoutOrder/complainDetail',
	            buttons:[{
	                text:'取消 ',
	                align:'left',
	                iconCls:'icon-cancel',
	            ...
	            buttons:[{
	                ...
	                handler:function(){
	                    $('#qcDealForm').form('submit',{
	                        url:"${rc.contextPath}/timeoutOrder/timeoutOrderQcDeal",
	                        onSubmit:function(){
	            ...
	            buttons:[{
	                ...
	                handler:function(){
	                    $('#qcBatchDealForm').form('submit',{
	                        url:"${rc.contextPath}/timeoutOrder/timeoutOrderQcBatchDeal",
	                        onSubmit:function(){
			...
			$("#qcDealForm_reason").combobox({
            	url:'${rc.contextPath}/timeoutOrder/jsonTimeoutReasonCode?isAvailable=1',
                valueField:'code',
                textField:'text'
            });
			...
			$('#s_data') .datagrid(
				{
					...
					collapsible : true,
					idField : 'id',
					url : '${rc.contextPath}/timeoutOrder/jsonOrderInfo',
					queryParams: {
	                    sellerId: '${sellerId!"0"}',
					...
					columns : [ [
							...
							{field : 'number', title : '订单编号', width : 50, align : 'center',
								formatter : function(value, row, index) {
									return '<a target="_blank" href="${rc.contextPath}/order/detail/'+row.id+'">'+row.number+'</a>';
								}
							},
							...
							{field : 'hidden', title : '操作', width : 45, align : 'center',
								formatter : function(value, row, index) {
									if(parseInt(row.isTimeout)==1){
										var a = '';
										var b = '';
										if(parseInt(row.timeOutResultStatus) == 1){
											a = '处理中 | ';
										}else if(parseInt(row.timeOutResultStatus) == 2 || parseInt(row.timeOutResultStatus) == 3){
											a = '<a href="javaScript:;" onclick="complainDetail(' + row.id + ')">申诉明细</a> | ';
										}else{
											a = '未申诉 | ';
										}
										if(row.isSendTimeoutDeal == 1){
