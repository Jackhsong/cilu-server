...
$('#nodeId').val(nodeId);
function searchAll() {
	$('#searchDivForm').attr("action", "${rc.contextPath}/wechatGroupData/monthList?nodes="+nodeId).submit();
}

function exportAll() {
	$('#searchDivForm').attr("action", "${rc.contextPath}/wechatGroupData/exportMonthList").submit();
}
</script>
