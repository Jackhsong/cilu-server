...
<div data-options="region:'center',title:'content'" style="padding:5px;">
	...
	<div id="searchDiv" class="datagrid-toolbar" style="height: 30px; padding: 15px">
		<form id="searchDivForm" action="${rc.contextPath}/analyze/userFirstBehaviorAnalyze" method="post">
			<table>
				<tr>
					<td>前一段时间：</td>
					<td>
						<input value="${prevStartTime}" id="prevStartTime" name="prevStartTime" onClick="WdatePicker({dateFmt: 'yyyy-MM-dd',maxDate:'#F{$dp.$D(\'prevEndTime\')}'})"/>
						~
...
function searchAll() {
	$('#searchDivForm').attr("action", "userFirstBehaviorAnalyze").submit();
}

function exportAll() {
	$('#searchDivForm').attr("action", "exportUserFirstBehaviorAnalyze").submit();
}
