...
<div data-options="region:'center',title:'content'" style="padding:5px;">
	...
	<div id="searchDiv" class="datagrid-toolbar" style="height: 30px; padding: 15px">
		<form id="searchDivForm" action="${rc.contextPath}/analyze/firstCategoryTurnover" method="post">
			<table>
				<tr>
					<td>查询日期</td>
...
function searchAll() {
	$('#searchDivForm').attr("action", "firstCategoryTurnover").submit();
}

function exportAll() {
	$('#searchDivForm').attr("action", "exportFirstCategoryTurnover").submit();
}
</script>
