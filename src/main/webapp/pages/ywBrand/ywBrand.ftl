    	  ...
    	  <!-- 新增品牌begin -->
		    <div id="brandDiv" class="easyui-dialog" style="width:600px;height:600px;padding:15px 20px;">
		        <form id="brandForm" method="post">
					...
					<input id="brand_form_category_id" type="hidden" name="ywBrandCategoryId" value="" >
...
<script>
	...
	function editForm(index) {
		...
		var ywBrandCategoryId = arr.rows[index].ywBrandCategoryId;
    	...
    	$('#brand_form_category_id').val(ywBrandCategoryId);
	...
	function display(id,isDisplay,order){
		...
		$.messager.confirm("提示信息",tip,function(ra){
	        if(ra){
		            $.messager.progress();
		            $.post(
		            		"${rc.contextPath}/ywBrand/updateBrandDisplay",
	...
	$(function(){
		...
		$('#s_data').datagrid({
            ...
            collapsible:true,
            idField:'id',
            url:'${rc.contextPath}/ywBrand/jsonBrandInfo',
            loadMsg:'正在装载数据...',
            singleSelect:false,
            fitColumns:true,
            remoteSort: true,
	    	...
	    	buttons: [
	    	{
	    		...
	    		handler: function(){
	    			$('#brandForm').form('submit',{
	    				url: "${rc.contextPath}/ywBrand/saveOrUpdate",
	    				onSubmit:function(){
	    					...
	    					var ywBrandCategoryId = $("#brandCategoryId").val();
	    					$("#brand_form_category_id").val(ywBrandCategoryId);
	    					if($.trim(name)==''){
