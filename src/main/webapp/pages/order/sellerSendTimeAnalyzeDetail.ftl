...
<body class="easyui-layout">
	...
	<div data-options="region:'center'" style="padding: 5px;">
		<div id="cc" class="easyui-layout" data-options="fit:true" >
			<div data-options="region:'north',title:'条件筛选-订单管理',split:true" style="height: 100px;">
				<form action="${rc.contextPath}/order/exportSellerSendTimeAnalyzeDetail" id="searchForm" method="post" >
						<input type="hidden" id="payTime" name="payTime" value="${payTime}"/>
						<input type="hidden" id="beginHour" name="beginHour" value="${beginHour}"/>
						<input type="hidden" id="endHour" name="endHour" value="${endHour}"/>
						<input type="hidden" id="sellerId" name="sellerId" value="${sellerId!''}"/>
						<table>
							<tr>
								<td>${payTime}商家付款订单发货明细&nbsp;&nbsp;&nbsp;</td>
								<td>
	...
	<script>
		...
		$(function() {
			...
			$('#s_data') .datagrid(
			{
				...
				url : '${rc.contextPath}/order/jsonSellerSendTimeAnalyzeDetail',
				loadMsg : '正在装载数据...',
				fitColumns : true,
				remoteSort : true,
				singleSelect : true,
