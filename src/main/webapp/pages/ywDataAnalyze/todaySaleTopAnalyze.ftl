...
<div data-options="region:'center',title:'content'" style="padding:5px;">
	...
	<div id="searchDiv" class="datagrid-toolbar" style="height: 30px; padding: 15px">
		<form id="searchDivForm" action="${rc.contextPath}/analyze/monthAnalyze" method="post">
			<table>
				<tr>
					<input type="hidden" name="selectDate" id="selectDate" >
					<td>查询日期：<span id="now">${now}</span></td>
					...
					<!-- <td>查询日期</td> -->
					<!-- <td><input value="" id="selectDate" name="selectDate" onClick="WdatePicker({dateFmt: 'yyyy-MM',minDate:'2015-03'})"/></td> -->
...
function exportAll() {
	$("#selectDate").val($("#now").text());
	$('#searchDivForm').attr("action", "exportTodaySaleTop").submit();
}
</script>
<script>
		var data1 = ${yesterdaySaleData};
		var data2 = ${nowSaleData};
		var lineChartData = {
