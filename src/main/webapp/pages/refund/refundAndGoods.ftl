...
function agreeRefund(step){
	...
	if(step == 4){
		...
		if(cardInfo=='' || cardInfo==null || cardInfo==undefined){
			$.messager.alert("提示",'请选择打款账户。如果没有可供选择的打款账户，请先去"售后管理-退款退货管理-财务打款账户管理"添加账户',"info");
			return false;
		}
	}
	if(step == 1) {
		if($('#refundReasonId').combobox('getValue') == '') {
			$.messager.alert("提示",'退款原因没有选择',"info");
			return false;
		}
	}
	...
	if(reg.test(money) || (step == 3) || (step == 4)){
		$.messager.confirm('info','请确认？',function(b){
			if(b){
				var tempData;
				if(step == 1) {
					tempData = {'id':id,'remark':remark,'step':step,'money':money,cardInfo:cardInfo,'modifyRefundType':modifyRefundType,type:2,'refundReasonId':$('#refundReasonId').combobox('getValue')};
				} else {
