...
function strToDate(str) {
 ...
 return date;
	...
	function edit(id) {
		...
		$('#display').hide();
		$.ajax({
			url: '${rc.contextPath}/ywError/findById?id=' + id,
			type:"POST",
			async:false,
			success: function(data) {
				$.messager.progress('close');
	...
	function refuse(id,accountId,withdrawals,type){
	    $.messager.confirm("提示信息","确定拒绝么？",function(re){
	        if(re){
	            $.messager.progress();
	            $.post("${rc.contextPath}/ywError/refuse",{id:id,accountId:accountId,withdrawals:withdrawals,type:type},function(data){
	                $.messager.progress('close');
	                if(data.status == 1){
	...
	function accept(id,accountId,withdrawals,type){
    	$.messager.confirm("提示信息","确定打款么？",function(re){
        	if(re){
	            $.messager.progress();
	            $.post("${rc.contextPath}/ywError/accept",{id:id,accountId:accountId,withdrawals:withdrawals,type:type},function(data){
	                $.messager.progress('close');
	                if(data.status == 1){
	...
	function updateActivity(row){
		$.ajax({
			url: '${rc.contextPath}/ywError/saveOrUpdate',
			type:"POST",
	...
	$(function() {
		...
		$('#productBaseId').blur(function() {
			if($('#productBaseId').val().length < 1)
				return;
			$.ajax({
                url: '${rc.contextPath}/ywError/getProductInfo',
                type: 'POST',
		...
		$('#s_data').datagrid({
            ...
            collapsible:true,
            idField:'id',
            url:'${rc.contextPath}/ywError/listInfo',
            loadMsg:'正在装载数据...',
            singleSelect:false,
            fitColumns:true,
            remoteSort: true,
					...
					$.messager.confirm("提示信息","确定新增关系么？",function(re){
        				if(re){
							...
							$('#editCustomGGRecommendForm').form('submit', {
								url : "${rc.contextPath}/ywError/save",
								onSubmit : function() {
										return valid();
								},
								success : function(data) {
