...
<div data-options="region:'center',title:'品类馆特卖',split:true" style="padding:5px;">

	<div id="searchDiv" class="datagrid-toolbar"  style="height: 120px;padding: 15px">
		<br/>
        ...
        <input id="categoryFirstId" type="hidden" name="categoryFirstId" value="${(categoryFirstId)!}" >
            ...
            <table>
            	<tr>
                    ...
                    <select id="saleStatus" name="saleStatus" style="width:173px;">
                    	...
                    	<option value="2" selected="selected">进行中（早场和晚场）</option>
                    ...
                    <select id="type" name="type" style="width:173px;">
                    	...
                    	<option value="3">自定义活动</option>
                    	<option value="4">原生自定义页面</option>
                    </select>
                    </td>
                    <td>&nbsp;特卖名称：</td>
                    <td><input id="name" name="name" value="" /></td>
                    <td>&nbsp;开售档期：</td>
                    <td><input id="startTime" name="startTime" value="" onClick="WdatePicker({dateFmt: 'yyyyMMdd'})" /></td>
                    <td>&nbsp;商家：</td>
...
<script>
	...
	function searchSaleWindow(){
    	$('#s_data').datagrid('load',{
			...
			categoryFirstId:$("#categoryFirstId").val()
    	});
	}
	...
	function updateActivity(row){
		$.ajax({
			url: '${rc.contextPath}/categorySale/updateCategoryOrderAndIndex',
			type:"POST",
			data: {id:row.id,categoryOrder:row.categoryOrder,categoryLockIndex:row.categoryLockIndex},
	...
	function editIt(index){
		...
		var urlStr = "${rc.contextPath}/categorySale/edit/"+arr.rows[index].id+"/"+${modelId}+"/"+${pageId}+"/"+${categoryFirstId};
	...
	function displayIt(id,code){
    	$.messager.confirm("提示信息","确定修改展现状态吗？",function(re){
        	if(re){
            	$.messager.progress();
            	$.post("${rc.contextPath}/categorySale/updateDisplayCode",{id:id,code:code},function(data){
	...
	function deleteIt(id){
		$.messager.confirm("提示","确定删除吗？",function(r){
			if(r){
				$.ajax({
					url:'${rc.contextPath}/categorySale/deleteCategorySale',
					type:'POST',
					data:{id:id},
					dataType:'json',
	...
	$(function(){
		$('#s_data').datagrid({
            ...
            collapsible:true,
            idField:'id',
            url:'${rc.contextPath}/categorySale/jsonCategorySaleWindowInfo',
            loadMsg:'正在装载数据...',
            queryParams:{
            ...
            columns:[[
            	...
            	{field:'saleType',    title:'特卖类型', width:30, align:'center'},
                ...
                {field:'saleTimeType',    title:'场次', width:30, align:'center'},
                ...
                {field:'categoryOrder',    title:'排序值', width:30, align:'center',editor:{type:'validatebox',options:{required:true}}},
                {field:'categoryLockIndex',    title:'锁定序位', width:30, align:'center',editor:{type:'validatebox',options:{required:true}}},
                {field:'hidden',  title:'操作', width:80,align:'center',
                    formatter:function(value,row,index){
                        if (row.editing) {
							var save = '<a href="javascript:void(0);" onclick="saverow('+index+')">保存</a> | ';
                        	var cancel = '<a href="javascript:void(0);" onclick="cancelrow('+index+')">取消</a>';
                        	return save+cancel;
						} else {
							var a = '';
							if(row.type == 1 || row.type == 2){
								a = '<a href="'+row.url+'" target="_blank">预览</a> | ';
							}
            ...
            toolbar:[{
                ...
                iconCls:'icon-add',
                handler:function(){
                    window.location.href = "${rc.contextPath}/categorySale/addSale/${modelId}/${pageId}/${categoryFirstId}"
                }
            }],
