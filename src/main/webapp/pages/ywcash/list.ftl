...
<div data-options="region:'center'" style="padding:5px;">
	<div id="cc" class="easyui-layout" data-options="fit:true" >
		<div data-options="region:'north',title:'提现管理',split:true" style="height: 90px;">
...
function strToDate(str) {
 ...
 return date;
	...
	function edit(id) {
		...
		$('#display').hide();
		$.ajax({
			url: '${rc.contextPath}/ywCash/findById?id=' + id,
			type:"POST",
			async:false,
			success: function(data) {
	...
	function refuse(id,type){
	    $.messager.confirm("提示信息","确定拒绝么？",function(re){
	        if(re){
	            $.messager.progress();
	            $.post("${rc.contextPath}/ywCash/refuse",{id:id,type:type},function(data){
	                $.messager.progress('close');
	                if(data.status == 1){
	...
	function accept(id,type){
    	$.messager.confirm("提示信息","确定打款么？",function(re){
        	if(re){
	            $.messager.progress();
	            $.post("${rc.contextPath}/ywCash/accept",{id:id,type:type},function(data){
	                $.messager.progress('close');
	                if(data.status == 1){
	...
	function updateActivity(row){
		$.ajax({
			url: '${rc.contextPath}/ywCash/saveOrUpdate',
			type:"POST",
	...
	$(function() {
		...
		$('#productBaseId').blur(function() {
			if($('#productBaseId').val().length < 1)
				return;
			$.ajax({
                url: '${rc.contextPath}/ywCash/getProductInfo',
                type: 'POST',
		...
		$('#s_data').datagrid({
            ...
            collapsible:true,
            idField:'id',
            url:'${rc.contextPath}/ywCash/listInfo',
            loadMsg:'正在装载数据...',
            singleSelect:false,
            fitColumns:true,
            remoteSort: true,
            ...
            columns:[[
				...
				'-',{
                    ...
                    handler: function(){
                        ...
                        if(rows.length > 0){
                            $.messager.confirm('打款','确定批量打款吗',function(b){
                                if(b){
                                	$.messager.progress();
                                	var type = "delete";
                                    var ids = [];
                                    ...
                                    $.ajax({
                    					type: 'post',
                    					url: '${rc.contextPath}/ywCash/deleteBat',
                    					data: {'ids': ids.join(",")},
                    					datatype:'json',
                    					success: function(data){
                    						$.messager.progress('close');
                            ...
                            $.messager.alert('提示','请选择中要提款的提现编号',"error");
                        }
                    }
			} ],
