...
<div data-options="region:'center'" style="padding:5px;">
	<div id="cc" class="easyui-layout" data-options="fit:true" >
		<div data-options="region:'north',title:'品类馆自定义版块管理',split:true" style="height: 120px;">
			<br/>
...
<script>
	...
	function updateActivity(row){
		$.ajax({
			url: '${rc.contextPath}/categoryRegion/updateCustomRegionSequence',
			type:"POST",
	...
	function displayIt(id,code){
    	...
    	$.messager.confirm("提示信息",tips,function(re){
        	if(re){
            	$.messager.progress();
            	$.post("${rc.contextPath}/categoryRegion/updateCustomRegionDisplayStatus",{id:id,code:code},function(data){
	...
	function availableIt(id,code){
    	...
    	$.messager.confirm("提示信息",tips,function(re){
        	if(re){
            	$.messager.progress();
            	$.post("${rc.contextPath}/categoryRegion/updateCustomRegionAvailableStatus",{id:id,code:code},function(data){
                	$.messager.progress('close');
                	if(data.status == 1){
                        $('#s_data').datagrid('reload');
                	} else{
	...
	$(function(){
		...
		$('#s_data').datagrid({
            ...
            collapsible:true,
            idField:'id',
            url:'${rc.contextPath}/categoryRegion/jsonCustomRegionInfo',
            loadMsg:'正在装载数据...',
            queryParams:{
                ...
                {field:'hidden',  title:'操作', width:80,align:'center',
                    formatter:function(value,row,index){
                    	if (row.editing){
                            ...
                            var c = '<a target="_blank" href="${rc.contextPath}/categoryRegion/manageLayout/' + row.id + '/'+${modelId}+'/'+${pageId}+'">管理布局</a> | ';
                            var d = '';
                            var e = '';
                            if(row.isDisplay == 1){
                    ...
                    if(rows.length > 0){
                        $.messager.confirm('展现','确认全部设为可用吗？',function(b){
                            if(b){
                                ...
                                $.post("${rc.contextPath}/categoryRegion/updateCustomRegionAvailableStatus",
									{id: ids.join(","),code:1},
									function(data){
										$.messager.progress('close');
										if(data.status == 1){
                    ...
                    if(rows.length > 0){
                        $.messager.confirm('展现','确认全部设为可见吗？',function(b){
                            if(b){
                                ...
                                $.post("${rc.contextPath}/categoryRegion/updateCustomRegionDisplayStatus",
									{id: ids.join(","),code:1},
									function(data){
										$.messager.progress('close');
										if(data.status == 1){
	    	...
	    	buttons: [
	    	{
	    		...
	    		handler: function(){
	    			$('#editCustomRegionForm').form('submit',{
	    				url: "${rc.contextPath}/categoryRegion/saveOrUpdateCustonRegion",
	    				onSubmit:function(){
	    					$.messager.progress();
	    				},
	    				success: function(data){
