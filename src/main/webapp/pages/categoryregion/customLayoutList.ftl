...
<script>
	...
	function updateActivity(row){
		$.ajax({
			url: '${rc.contextPath}/categoryRegion/updateCustomLayoutSequence',
			type:"POST",
	...
	function displayIt(layoutId,code){
    	...
    	$.messager.confirm("提示信息",tips,function(re){
        	if(re){
            	$.messager.progress();
            	$.post("${rc.contextPath}/categoryRegion/updateCustomLayoutDisplayStatus",{layoutId:layoutId,code:code},function(data){
	...
	function deleteIt(id){
		$.messager.confirm("提示信息","确定删除吗？",function(re){
        	if(re){
        		$.messager.progress();
        		$.ajax({
        			url: '${rc.contextPath}/categoryRegion/deleteCustomLayout',
        			type:"POST",
        			data: {id:id},
        			success: function(data) {
        				$.messager.progress('close');
	...
	$(function(){
		...
		$('#s_data').datagrid({
            ...
            collapsible:true,
            idField:'id',
            url:'${rc.contextPath}/categoryRegion/jsonCustomLayoutInfo',
            loadMsg:'正在装载数据...',
            fitColumns:true,
            remoteSort: true,
            singleSelect:true,
                ...
                {field:'hidden1',  title:'操作', width:70,align:'center',
                    formatter:function(value,row,index){
                    	if (row.editing){
                    		...
                    		var b = '<a href="${rc.contextPath}/categoryRegion/toEdit/${regionId}/' + row.id + '/'+${modelId}+'/'+${pageId}+'")>编辑</a> | ';
            ...
            toolbar:[{
                ...
                iconCls:'icon-back',
                handler:function(){
                	window.location.href = "${rc.contextPath}/categoryRegion/listCategoryRegion/${modelId}/${pageId}"
                }
            },'-',{
                id:'_add',
                text:'新增布局',
                iconCls:'icon-add',
                handler:function(){
                	window.location.href = "${rc.contextPath}/categoryRegion/toAdd/${regionId}/${modelId}/${pageId}"
                }
            }],
