...
<div data-options="region:'center',title:'添加特卖信息'" style="padding:5px;">
	...
	<form id="saveSaleWindow" action="${rc.contextPath}/ywSale/save"  method="post">
		<fieldset>
...
<script type="text/javascript">
	...
	function updateActivity(row){
		$.ajax({
			url: '${rc.contextPath}/ywSale/updateProductTime',
			type:"POST",
	...
	function checkFormDate(type, subjectId){
		...
		$.post("${rc.contextPath}/ywSale/checkProductTime",
				{type: type,subjectId:subjectId,startTime:startTime,endTime:endTime},
				function(data){
					if(data.status == 1){
						if(!($.trim(name) == '' && $.trim(desc) == '')){
							$.messager.progress();
							$.post("${rc.contextPath}/ywSale/checkNameAndDescLength",
									{name: name,desc:desc},
									function(data1){
										if(data1.status != 1){
	...
	$(function(){
		...
		$('#s_data').datagrid({
            ...
            collapsible:true,
            idField:'id',
            url:'${rc.contextPath}/ywSale/jsonProductInfo',
            queryParams: {
				type: 0
			},
            loadMsg:'正在装载数据...',
                        ...
                        if(rows.length < 1){
                            ...
                            $.messager.confirm('添加商品','请确认',function(b){
                                if(b){
                                    ...
                                    $.post("${rc.contextPath}/ywSale/updateProductTime",
