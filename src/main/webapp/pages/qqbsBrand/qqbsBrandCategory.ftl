...
<script>
	...
	function display(id,isDisplay,order){
		...
		$.messager.confirm("提示信息",tip,function(ra){
	        if(ra){
		            $.messager.progress();
		            $.post(
		            		"${rc.contextPath}/qqbsBrandCategory/updateCategoryDisplay",
	...
	$(function(){
		$('#s_data').datagrid({
            ...
            collapsible:true,
            idField:'id',
            url:'${rc.contextPath}/qqbsBrandCategory/jsonBrandCategoryInfo',
            loadMsg:'正在装载数据...',
            singleSelect:false,
            fitColumns:true,
            remoteSort: true,
            ...
            columns:[[
                ...
                {field:'hidden',  title:'操作', width:80,align:'center',
                    formatter:function(value,row,index){
	                	...
	                	var b = '<a target="_blank" href="${rc.contextPath}/qqbsBrand/brandManage/'+row.id+'">管理品牌</a> | ';
	                	var c = '';
	                	if(row.isDisplay == 1){
	    	...
	    	buttons: [
	    	{
	    		...
	    		handler: function(){
	    			$('#brandCategoryForm').form('submit',{
	    				url: "${rc.contextPath}/qqbsBrandCategory/saveOrUpdate",
	    				onSubmit:function(){
