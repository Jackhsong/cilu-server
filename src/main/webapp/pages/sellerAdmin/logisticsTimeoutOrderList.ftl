...
<body class="easyui-layout">
	...
	<div data-options="region:'center'" style="padding: 5px;">
		<div id="cc" class="easyui-layout" data-options="fit:true" >
			<div data-options="region:'north',title:'物流时效管理',split:true" style="height: 190px;">
				<form action="${rc.contextPath}/sellerAdminOrder/exportUnSettlementOrder" id="searchForm" method="post" >
						...
						<input type="hidden" name="orderIdList" id="orderIdList" value="${orderIdList!''}">
						<table class="search">
							<tr>
								<td class="searchName">付款时间：</td>
								<td class="searchText">
                                    <input value="" id="payTimeBegin" name="payTimeBegin" onClick="WdatePicker({dateFmt: 'yyyy-MM-dd HH:mm:ss'})"/>
                                    ~
                                    <input value="" id="payTimeEnd" name="payTimeEnd" onClick="WdatePicker({dateFmt: 'yyyy-MM-dd HH:mm:ss'})"/>
								</td>
								<td class="searchName">发货时间：</td>
								<td class="searchText">
							...
							<tr>
                                <td class="searchName">物流时效是否超时：</td>
                                <td class="searchText">
	...
	<script>
		...
		function complain(index){
			...
			var arr=$("#s_data").datagrid("getData");
			$('#complainForm_orderId').val(arr.rows[index].orderId);
...
//			});
            ...
            buttons:[{
                ...
                handler:function(){
                    $('#complainForm').form('submit',{
                        url:"${rc.contextPath}/sellerAdminOrder/logisticsTimeOutComplain",
                        onSubmit:function(){
			...
			$('#s_data') .datagrid(
			{
				...
				collapsible : true,
				idField : 'orderId',
				url : '${rc.contextPath}/sellerAdminOrder/jsonLogisticsTimeoutOrderInfo',
				queryParams: {
                    sellerId: '${sellerId!"0"}',
				...
				columns : [ [
						...
						{field : 'isTimeout', title : '物流时效<br/>是否超时', width : 30, align : 'center',
						...
						{field : 'number', title : '订单编号', width : 50, align : 'center',
							formatter : function(value, row, index) {
								var a = '<a target="_blank" href="${rc.contextPath}/sellerAdminOrder/detail/'+row.orderId+'">'+row.number+'</a>';
								return a;
							}
						},
