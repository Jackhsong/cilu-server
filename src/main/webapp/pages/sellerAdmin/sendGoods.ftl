...
<div data-options="region:'center',title:'订单发货'" style="padding:5px;">
	<div>
		<div style="padding-left:500px;padding-bottom:15px;">
			<table>
				<tr>
					<td>
                        <form action="${rc.contextPath}/sellerAdminOrder/exportSendTemplate" method="post">
                            <input type=hidden name="type" value="1" />
                            <input type="submit" value="下载发货模板" style="width:100px"/>
                        </form>
					</td>
                    <td>
                        <form action="${rc.contextPath}/sellerAdminOrder/exportPostageTemplate" method="post">
                            <input type=hidden name="type" value="1" />
                            &nbsp;&nbsp;&nbsp;<input type="submit" value="下载支持物流公司" style="width:120px"/>
                        </form>
					</td>
				</tr>
			</table>
		...
		<form action="${rc.contextPath}/sellerAdminOrder/exportSendResult" method="post">
			<input type=hidden name="type" value="1" />
...
<script>
	...
	$(function(){
		...
		$('#uploadOrderForm').form({
			url:'${rc.contextPath}/sellerAdminOrder/batchSendOrder',
			onSubmit: function(){
				// do some check   
		...
		$('#s_data').datagrid({
			...
			collapsible : true,
			idField : 'pid',
			url : '${rc.contextPath}/sellerAdminOrder/receiveResult',
			queryParams : {
				delete:1
			},
			loadMsg : '正在装载数据...',
