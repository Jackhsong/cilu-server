...
<div data-options="region:'center'" style="padding:5px;">
	<div id="cc" class="easyui-layout" data-options="fit:true" >
		<div data-options="region:'north',title:'格格推荐-新版本',split:true" style="height: 90px;">
...
function checkTwo() {
	...
	function edit(id) {
		...
		$('#display').hide();
		$.ajax({
			url: '${rc.contextPath}/customGGRecommend24/findById?id=' + id,
			type:"POST",
			async:false,
			success: function(data) {
				...
				if(data.status == 1) {
					...
					if(data.data.displayType == 2) {
						$('#two').show();
						$('#twoTip').text('宽度：376px');
						$('#oneTip').text('宽度：376px');
	...
	function updateActivity(row){
		$.ajax({
			url: '${rc.contextPath}/customGGRecommend24/saveOrUpdate',
			type:"POST",
	...
	$(function() {
		...
		$('#s_data').datagrid({
            ...
            collapsible:true,
            idField:'id',
            url:'${rc.contextPath}/customGGRecommend24/listInfo',
            loadMsg:'正在装载数据...',
            singleSelect:false,
            fitColumns:true,
            remoteSort: true,
