...
function strToDate(str) {
 ...
 return date;
	...
	function edit(id) {
		...
		$('#display').hide();
		$.ajax({
			url: '${rc.contextPath}/qqbsCash/findById?id=' + id,
			type:"POST",
			async:false,
			success: function(data) {
	...
	function updateActivity(row){
		$.ajax({
			url: '${rc.contextPath}/qqbsCash/saveOrUpdate',
			type:"POST",
	...
	$(function() {
		...
		$('#productBaseId').blur(function() {
			if($('#productBaseId').val().length < 1)
				return;
			$.ajax({
                url: '${rc.contextPath}/qqbsCash/getProductInfo',
                type: 'POST',
                        ...
                        if(rows.length > 0){
                            $.messager.confirm('打款','确定批量打款吗',function(b){
                                if(b){
                                    ...
                                    $.ajax({
                    					type: 'post',
                    					url: '${rc.contextPath}/qqbsCash/deleteBat',
                    					data: {'ids': ids.join(",")},
                    					datatype:'json',
                    					success: function(data){
                    						$.messager.progress('close');
			...
			buttons : [ {
				...
				handler : function() {
					$('#editCustomGGRecommendForm').form('submit', {
						url : "${rc.contextPath}/qqbsCash/save",
						onSubmit : function() {
							return valid();
						},
						success : function(data) {
