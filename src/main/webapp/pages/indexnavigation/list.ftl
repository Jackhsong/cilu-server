...
<div data-options="region:'center'" style="padding:5px;">
	<div id="cc" class="easyui-layout" data-options="fit:true" >
		<div data-options="region:'north',title:'新版首页自定义导航',split:true" style="height: 90px;">
		    ...
		    <div id="editCustomNavigationDiv" class="easyui-dialog" style="width:500px;height:300px;padding:15px 20px;">
		        <form id="editCustomNavigationForm" method="post">
					...
					<p>
						...
						<span><input type="radio" name="customNavigationType" value="4"/>品类馆&nbsp;</span>
					</p>
					...
					<p id="page2">
						<span>品 类 馆：</span>
						<span><input type="text" name="pageId2" id="pageId2" style="width: 200px;"/></span>
					</p>
					<p id="display">
						<span>展现状态：</span>
...
<script>
    function editArea(id){
        var url = $("#contextPath").val() + "/indexNavigation/editAreaForm?id=" + id;
...
$(function(){
	...
	function edit(id) {
		...
		$("#pageId2").combobox({
            url:'${rc.contextPath}/cateGoryManager/ajaxAppCustomPage',
            editable : false,
            valueField:'code',
            textField:'text',
        });
		$.ajax({
			url: '${rc.contextPath}/indexNavigation/findById?id=' + id,
			type:"POST",
			sync:false,
			success: function(data) {
				...
				if(data.status == 1) {
					...
					}else if(data.data.customNavigationType == 4){
						$('#page2').show();
						$('#pageId2').combobox('setValue', data.data.displayId);
					}
	...
	function updateActivity(row){
		$.ajax({
			url: '${rc.contextPath}/indexNavigation/updateByParam',
			type:"POST",
	...
	$(function(){
		...
		$('#s_data').datagrid({
            ...
            collapsible:true,
            idField:'id',
            url:'${rc.contextPath}/indexNavigation/info',
            loadMsg:'正在装载数据...',
            singleSelect:false,
            fitColumns:true,
            remoteSort: true,
            ...
            columns:[[
                ...
                {field:'name',    title:'导航名称', width:70, align:'center'},
                {field:'typeName',    title:'关联类型', width:70, align:'center'},
        ...
        $('#editArea_div').dialog({
            ...
            height: 600,
            closed: true,
            href: '${rc.contextPath}/indexNavigation/editArea',
            buttons:[{
                text:'关闭    ',
                align:'left',
                iconCls:'icon-cancel',
			...
			buttons : [ {
				...
				handler : function() {
					$('#editCustomNavigationForm').form('submit', {
						url : "${rc.contextPath}/indexNavigation/saveOrUpdate",
						onSubmit : function() {
							return valid(1);
						},
						success : function(data) {
