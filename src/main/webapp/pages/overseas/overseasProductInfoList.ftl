...
<div data-options="region:'center',title:'content'" style="padding: 5px;">
	<div title="海外购商品导单名称和价格管理" class="easyui-panel" style="padding: 10px">
		<div id="searchDiv" class="datagrid-toolbar" style="height: 50px; padding: 15px">
			<form action="${rc.contextPath}/overseasOrder/exportOverseasProductInfo" id="searchForm" method="post">
				<table>
					<tr>
						<td>
							商品编码：
			...
			<div style="display:none">
				<div>
					<form action="${rc.contextPath}/overseasOrder/exportImportOverseasProTemplate" id="searchForm" method="post">
	...
	<div id="updateProduct" class="easyui-dialog" style="width:450px;height:370px;padding:20px 20px;">
        <form id="updateProduct_form" method="post">
            ...
            <table>
                ...
                <tr id="sellerIdTr" style="display:none">
					<td>所属商家:</td>
					<td><input id="sellerId" name="sellerId" value="" style="width:280px" /></td>
				</tr>
				<tr id="sellerNameTr" style="display:none">
					<td>所属商家:</td>
					<td><input id="sellerName" name="sellerName" value="" readonly style="width:280px" /></td>
				</tr>
                <tr>
                    <td>原名称:</td>
...
function deleteIt(id){
    $.messager.confirm("提示信息","确定删除么？",function(re){
        if(re){
            $.messager.progress();
            $.post("${rc.contextPath}/overseasOrder/deleteProduct",{id:id},function(data){
...
function editProduct(index){
	...
	$("#sellerName").val(arr.rows[index].sellerName);
...
$(function(){
	...
	$('#s_data') .datagrid({
		...
		collapsible : true,
		idField : 'id',
		url : '${rc.contextPath}/overseasOrder/jsonOverseasProductInfo',
		loadMsg : '正在装载数据...',
		fitColumns : true,
		remoteSort : true,
		singleSelect : true,
		...
		columns : [ [
			...
			{field : 'exportPrice', title : '导单售价', width : 40, align : 'center'},
			{field : 'orderNumber', title : '来源订单号', width : 50, align : 'center'},
			{field : 'sellerName', title : '商家', width : 30, align : 'center'},
			{field : 'sendAddress', title : '发货地', width : 60, align : 'center'},
        ...
        buttons:[{
            ...
            handler:function(){
            	var params = {
            			...
            			sellerId:$('#sellerId').combobox('getValue'),//商家
            	...
            	}else if(params.id == '' && (params.sellerId == '' || params.sellerId == null || params.sellerId == undefined)){
            		$.messager.alert("提示","请从下拉列表中选择商家","error");
            	}else{
            		$.messager.progress();
                	$.ajax({
                		url:"${rc.contextPath}/overseasOrder/saveProduct",
    	    			type: 'post',
    	    			dataType: 'json',
    	    			data: params,
	...
	$('#importForm').form({
		url:'${rc.contextPath}/overseasOrder/importOverseasPro',   
		onSubmit: function(){   
			// do some check   
