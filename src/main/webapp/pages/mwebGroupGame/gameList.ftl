...
<div data-options="region:'center',title:''" style="padding:5px;">
	...
	<div id="cc" class="easyui-layout" data-options="fit:true" >
		...
		<div data-options="region:'north',title:'条件筛选-心动慈露游戏管理',split:true" style="height: 120px;">
		...
		<!-- dialog begin -->
		<div id="addGameDiv" class="easyui-dialog" style="width:600px;height:420px;padding:10px 10px;">
			<form action="" id="gameForm" method="post">
				...
				<p>
					<span>团购商品Id：</span>
					<span><input type="text" id="gameForm_mwebGroupProductId" name="mwebGroupProductId" value="" style="width:300px;" onblur="stringTrim(this)"/></span>&nbsp;
					<span id="mwebGroupProductInfo" style="color:red"></span>
				</p>
				<p>
					<span>团购商品：</span>
					<span>
						<input type="text" id="gameForm_mwebGroupProductImage" name="mwebGroupProductImage" value="" style="width:300px;"/>
						<a onclick="picDialogOpen('gameForm_mwebGroupProductImage')" href="javascript:;" class="easyui-linkbutton">上传图片</a>
					</span>
		...
		<!-- dialog begin -->
		<div id="editGameDiv" class="easyui-dialog" style="width:620px;height:390px;padding:10px 10px;">
			<form action="" id="editGameForm" method="post">
				...
				<p>
					<span>团购商品Id：</span>
					<span><input type="text" id="editGameForm_mwebGroupProductId" name="mwebGroupProductId" value="" style="width:300px;" onblur="stringTrim(this)"/></span>&nbsp;
					<span id="mwebGroupProductInfo2" style="color:red"></span>
				</p>
				<p>
					<span>&nbsp;&nbsp;团购商品：</span>
					<span>
						<input type="text" id="editGameForm_mwebGroupProductImage" name="mwebGroupProductImage" value="" style="width:300px;"/>
						<a onclick="picDialogOpen('editGameForm_mwebGroupProductImage')" href="javascript:;" class="easyui-linkbutton">上传图片</a>
					</span>
				</p>
				<p>
					<span>&nbsp;&nbsp;游戏简介：</span>
...
function cleanAddGameDiv(){
	...
	$("#gameForm_mwebGroupProductId").val('');
...
function cleanEditGameDiv(){
	...
	$("#editGameForm_mwebGroupProductId").val('');
...
function editIt(index){
    ...
    $("#editGameForm_mwebGroupProductImage").val(arr.rows[index].mwebGroupProductImage);
    $("#editGameForm_mwebGroupProductId").val(arr.rows[index].mwebGroupProductId);
...
function updateAvailable(id, isAvailable){
	...
	$.messager.confirm("提示信息",tip,function(ra){
        if(ra){
	            $.messager.progress();
	            $.post(
	            		"${rc.contextPath}/mwebGroupGame/updateGameAvailable",
...
function updateSendMsg(index, status){
	...
	$.messager.confirm("提示信息",tip,function(ra){
        if(ra){
	            $.messager.progress();
	            $.post(
	            		"${rc.contextPath}/mwebGroupGame/updateGameSendMsg",
	            		{id:id,status:status},
	            		function(data){
	   	                $.messager.progress('close');
	   	                if(data.status == 1){
...
function deleteId(id){
	$.messager.confirm("提示信息","确定删除吗？",function(ra){
        if(ra){
	            $.messager.progress();
	            $.post(
	            		"${rc.contextPath}/mwebGroupGame/deleteGame",
	            		{id:id},
	            		function(data){
	   	                $.messager.progress('close');
	   	                if(data.status == 1){
...
$(function(){
	...
	$("#gameForm_mwebGroupProductId").change(function(){
	...
	$("#editGameForm_mwebGroupProductId").change(function(){
		var mwebGroupProductId = $.trim($(this).val());
		if(mwebGroupProductId != ''){
            $.post(
  	           	"${rc.contextPath}/wechatGroup/jsonProductInfoForTeamById",
    ...
    $('#s_data').datagrid({
        ...
        collapsible:true,
        idField:'id',
        url:'${rc.contextPath}/mwebGroupGame/jsonGameInfo',
        loadMsg:'正在装载数据...',
        fitColumns:true,
        remoteSort: true,
        singleSelect:true,
        ...
        columns: [
            [
                ...
                {field: 'mwebGroupProductImage_img', title: '团购商品', width: 40, align: 'center'},
    	...
    	buttons: [
    	{
    		...
    		handler: function(){
    			$('#gameForm').form('submit',{
    				url: "${rc.contextPath}/mwebGroupGame/saveGame",
    				onSubmit:function(){
    					...
    					var mwebGroupProductId= $("#gameForm_mwebGroupProductId").val();
    					if($.trim(gameName)==''){
    			...
    			$('#editGameForm').form('submit',{
    				...
    				onSubmit:function(){
    					...
    					if($.trim(gameName)==''){
    						...
    						$.messager.alert("info","输入团购id","warn");
    						return false;
    					}
    					$.messager.progress();
    				},
            ...
            handler:function(){
            	...
            	if($.trim(msgContent) == ''){
            		...
            		$.messager.alert("提示",'短信内容字数超过限制',"warning");
           			...
           			$.messager.progress();
       				$.ajax({
       		            url: '${rc.contextPath}/mwebGroupGame/editMsgContent',
       		            type: 'post',
       		            dataType: 'json',
