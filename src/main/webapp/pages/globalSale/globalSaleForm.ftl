...
<div data-options="region:'center',title:'添加特卖信息'" style="padding:5px;">
	...
	<form id="saveSaleWindow" action="${rc.contextPath}/globalSale/save"  method="post">
		<fieldset>
			<input type="hidden" value="<#if saleWindow.id?exists && (saleWindow.id != 0)>${saleWindow.id?c}</#if>" id="editId" name="editId" />
			<legend>特卖信息</legend>
			 特卖类型:<input type="radio" name="type" id="type1" value="1" checked>单品&nbsp;&nbsp;&nbsp;<br><br>
...
<!-- 			</#if> -->
    		 特卖名称: <input maxlength="45" id="name" name="name" style="width:300px;"  value="<#if saleWindow.name?exists>${saleWindow.name}</#if>" />
    		...
    		特卖图片: <input type="text" readonly id="image" name="image" style="width:450px;" value="<#if saleWindow.image?exists>${saleWindow.image}</#if>"/><br/><br/>

			 开始时间:<input value="<#if startTime?exists >${startTime}</#if>" id="startTime" name="startTime" onClick="WdatePicker({dateFmt: 'yyyy-MM-dd 10:00:00',maxDate:'#F{$dp.$D(\'endTime\')}'})"/>&nbsp;&nbsp;&nbsp;
			 结束时间:<input value="<#if endTime?exists >${endTime}</#if>" id="endTime" name="endTime" onClick="WdatePicker({dateFmt: 'yyyy-MM-dd 10:00:00',minDate:'#F{$dp.$D(\'startTime\')}'})"/>
			 <span style="color:red">必须</span><br/><br/>
...
<script type="text/javascript">
	...
	function updateActivity(row){
		$.ajax({
			url: '${rc.contextPath}/globalSale/updateProductTime',
			type:"POST",
	...
	function checkFormDate(type, subjectId){
		...
		$.post("${rc.contextPath}/globalSale/checkProductTime",
				{type: type,subjectId:subjectId,startTime:startTime,endTime:endTime},
				function(data){
					if(data.status == 1){
						if(!($.trim(name) == '' && $.trim(desc) == '')){
							$.messager.progress();
							$.post("${rc.contextPath}/globalSale/checkNameAndDescLength",
									{name: name,desc:desc},
									function(data1){
										if(data1.status != 1){
	...
	$(function(){
		...
		$('#s_data').datagrid({
            ...
            collapsible:true,
            idField:'id',
            url:'${rc.contextPath}/globalSale/jsonProductInfo',
            queryParams: {
				type: 0
			},
            loadMsg:'正在装载数据...',
                        ...
                        if(rows.length < 1){
                            ...
                            $.messager.confirm('添加商品','请确认',function(b){
                                if(b){
                                    ...
                                    $.post("${rc.contextPath}/globalSale/updateProductTime",
