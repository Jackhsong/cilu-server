...
<div data-options="region:'center',title:'content'" style="padding:5px;">
	...
	<div title="商品管理" class="easyui-panel" style="padding:10px">
   ...
   <div id="editSell_div" style="width:450px;height:250px;padding:20px 20px;">
    ...
    <div id="quickAddProductDiv" class="easyui-dialog" style="width:400px;height:200px;padding:10px 10px;">
    	<input type="hidden" id="quickAddProductDiv_type" value="${type}"/>
    	<input type="hidden" id="quickAddProductDiv_typeId" value="${typeId}"/>
        <table cellpadding="5">
            <tr>
...
function deleteIt(id){
    $.messager.confirm("提示信息","确定删除吗？",function(re){
        if(re){
            $.messager.progress();
    		$.ajax({
    			url: '${rc.contextPath}/activityManjian/deleteActivityManjianProduct',
    			type:"POST",
    			data: {ids:id},
    			success: function(data) {
    				if(data.status == 1){
	...
	$('#updatePrice').dialog({
		...
		$('#s_data').datagrid({
            ...
            collapsible:true,
            idField:'id',
            url:'${rc.contextPath}/activityManjian/jsonManjianProductInfo',
            queryParams: {
				type:${type},
				typeId:${typeId}
			},
            ...
            columns:[[
                ...
                {field:'isOffShelvesDesc',title:'商品状态', width:20, align:'center'},
                ...
                {field:'hidden',  title:'操作', width:80,align:'center',
                    formatter:function(value,row,index){
                   		var a = '<a href="javaScript:;" onclick="editProduct(' + index + ')">改商品</a>';
						var b = ' | <a href="javaScript:;" onclick="editPrice(' + index + ')">改价格</a>';
                       	...
                       	var e = ' | <a href="javaScript:;" onclick="editSell(' + index + ')">改销量</a>';
                   		var f = ' | <a href="javaScript:;" onclick="deleteIt(' + row.id + ')">删除</a>';
                       	return a+b+c+d+e+f;
                    }
                }
            ]],
            ...
            toolbar:[{
                ...
                handler:function(){
                 ...
                 var rows = $('#s_data').datagrid("getSelections");
                   ...
                   text:'导出商品',
                   handler: function(){
                   		var url = '${rc.contextPath}/activityManjian/exportProduct?type=${type}&typeId=${typeId}';
        	...
        	buttons: [
        	{
        		...
        		handler: function(){
                    ...
                    if(time_start != "" && time_edit != ""){
	        			$.ajax({
	        	            url: '${rc.contextPath}/activityManjian/updateProductTime',
	        	            type: 'post',
	        	            dataType: 'json',
	        	            data: {'productIds':productIds,'startTime':time_start,'endTime':time_edit},
	        	            success: function(data){
	        	            	$.messager.progress('close');
	        	                if(data.status == 1){
	        	                	$('#editStartTime').val('');
        ...
        $('#product_data').datagrid({
            ...
            collapsible:true,
            idField:'id',
            url:'${rc.contextPath}/activityManjian/jsonProductList',
            queryParams: {
				typeId:${typeId},
				status:0,
				type:${type}
   ...
   $('#add_div').dialog({
	...
	function addProduct(productIds){
		$.ajax({
	            url: '${rc.contextPath}/activityManjian/addProductForActivityManjian',
	            type: 'post',
	            dataType: 'json',
	            data: {'productIds':productIds,'type':${type}, 'typeId':${typeId}},
	            success: function(data){
	            	$.messager.progress('close');
	                if(data.status == 1){
	                	$('#quickAddProductDiv').dialog('close');
