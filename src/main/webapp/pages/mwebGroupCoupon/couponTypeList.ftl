...
<div data-options="region:'center',title:'心动慈露优惠券类型管理'" style="padding: 5px;">
	        ...
	        <form id="searchForm" action="" method="post" >
	        	<table>
					<tr>
						...
						<td>&nbsp;使用范围：</td>
						<td>
						   <select name="searchCouponScope" id="searchCouponScope">
			        ...
			        <table cellpadding="5">
			            <tr>
			             ...
			             <tr>
			                <td>拼团可用范围：</td>
			                <td>
			                	<input type="radio" name="teamType" id="teamType1" value="1" checked />仅用于开团&nbsp;
			                	<input type="radio" name="teamType" id="teamType2" value="2" />开团参团均可使用&nbsp;
			                </td>
			            </tr>
...
function clearCouponDetail(){
	$("#searchCouponType option:first").prop("selected", 'selected');
	$("#searchIsAvailable option:first").prop("selected", 'selected');
	$("#searchCouponScope option:first").prop("selected", 'selected');
	$('#s_data').datagrid('load',{});
}
...
function updateStatus(id, isAvailable){
	...
	$.messager.confirm('提示',confirm,function(r){
	    if (r){
	    	$.messager.progress();
			$.ajax({
		           url: '${rc.contextPath}/mwebGroupCoupon/updateStatus',
		           type: 'post',
		           dataType: 'json',
...
function cleanForm(){
    ...
    $("#addCouponTypeForm_scopeType").val(1);
	$("#productName").html('');
...
function modifyDetail(index){
    ...
    var teamType=arr.rows[index].teamType;
    ...
    $.ajax({
		type:'post',
		url:'${rc.contextPath}/mwebGroupCoupon/checkIsInUse',
		data:{'id':id},
		dataType:'json',
		success:function(data){
			if(data.status==1){
				...
				if(teamType==1){
				$("#teamType1").prop("checked", true);
				}
				else if(teamType==2){
				$("#teamType2").prop("checked", true);
				}
			    $("#addCouponTypeForm_scopeType").val(scopeType);
				$("#userScope").val(desc);
...
$(function(){
	...
	$("#productId").keyup(function(){
		...
		if(productId != ''){
			$.ajax({
				type:'post',
				url:'${rc.contextPath}/mwebGroupCoupon/showScopeTypeName',
	...
	$('#s_data').datagrid({
        ...
        collapsible:true,
        idField:'id',
        url:'${rc.contextPath}/mwebGroupCoupon/jsonCouponDetailInfo',
        loadMsg:'正在装载数据...',
        fitColumns:true,
        remoteSort: true,
        singleSelect:true,
        ...
        columns:[[
            ...
            {field:'status',     title:'使用状态',  width:20,  align:'center'},
            {field:'teamTypeStr',     title:'拼团可用范围',  width:30,  align:'center'},
            {field:'scope',    title:'优惠券商品使用范围', width:30, align:'center'},
    	...
    	buttons: [
    	{
    		...
    		handler: function(){
    			$('#addCouponTypeForm').form('submit',{
    				url: "${rc.contextPath}/mwebGroupCoupon/saveOrUpdate",
    				onSubmit:function(){
