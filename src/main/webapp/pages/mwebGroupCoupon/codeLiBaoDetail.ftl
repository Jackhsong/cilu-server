...
<div data-options="region:'center'" style="padding: 5px;">
	<div id="cc" class="easyui-layout" data-options="fit:true" >
		<div data-options="region:'north',title:'心动慈露优惠码详细--<#if te?exists && te==1>单次<#else>多次</#if>',split:true" style="height: 100px;padding-top:10px">
	        <form id="searchForm" action="${rc.contextPath}/mwebGroupCouponCode/exportCouponCodeLiBaoDetail" method="post" >
...
$(function(){
	...
	function showDetail(type,couponAccountId){
		var url = "${rc.contextPath}/mwebGroupCouponCode/couponCodeLiBaoDetail/"+type+"/"+couponAccountId;
	...
	$(function(){
		$('#s_data').datagrid({
	        ...
	        striped: true,
	        collapsible:true,
	        url:'${rc.contextPath}/mwebGroupCouponCode/jsonCouponCodeLiBaoDetail',
	        ...
	        columns:[[
                ...
                {field:'teamAccountId',    title:'心动慈露用户ID', width:30, align:'center'},
	            {field:'accountTypeStr',    title:'用户类型', width:30, align:'center'},
	            {field:'couponTypeStr',    title:'礼包信息', width:80, align:'center'},
	            ...
	            {field:'hidden',  title:'操作', width:40,align:'center',
	                formatter:function(value,row,index){
	                    var a = '';
	                    if(row.usedCode == 1){
	                    	a = '<a href="javaScript:;" onclick="showDetail(' + row.type + ','+ row.couponAccountId + ')">查看详细</a>';
	                    }
	                    return a;
	                }
	            }
