...
<div data-options="region:'center'" style="padding: 5px;">
	<div id="cc" class="easyui-layout" data-options="fit:true" >
		<div data-options="region:'north',title:'心动慈露优惠码管理',split:true" style="height: 100px;padding-top:10px">
...
$(function(){
	...
	function available(id,isAvailable){
		...
		$.messager.confirm('提示',confirm,function(r){
		    if (r){
		    	$.messager.progress();
				$.ajax({ 
				       url: '${rc.contextPath}/mwebGroupCouponCode/available',
				       type: 'post',
				       dataType: 'json',
	...
	function showDetail(id,type){
		var url = "${rc.contextPath}/mwebGroupCouponCode/couponCodeDetail/"+type+"/"+id;
		window.open(url,'_blank');
	}
	
	//查看礼包详情
	function showLiBaoDetail(id,type){
		var url = "${rc.contextPath}/mwebGroupCouponCode/couponCodeLiBaoDetail/"+type+"/"+id;
		window.open(url,'_blank');
	}
	...
	$(function(){
			    ...
			    handler:function(){
			    	...
			    	if(params.couponDetail == '' || typeof(params.couponDetail) == "undefined"){
			    		...
			    		$.messager.alert("提示","生成"+params.nums+"个优惠码耗时约"+sec+"秒","info",function(){
			    			$.messager.progress();
			    			$.ajax({
								url: '${rc.contextPath}/mwebGroupCouponCode/addCode',
								type: 'post',
								dataType: 'json',
								data: params,
								success: function(data){
		...
		$('#s_data').datagrid({
	        ...
	        collapsible:true,
	        idField:'id',
	        url:'${rc.contextPath}/mwebGroupCouponCode/jsonCouponCode',
	        loadMsg:'正在装载数据...',
	        fitColumns:true,
	        remoteSort: true,
	        singleSelect:true,
