        ...
        <form id="searchForm" method="post" >
            <table cellspacing="5" cellpadding="5">
            	<tr>
                    ...
                    <td>&nbsp;&nbsp;&nbsp;Banner进行状态：</td>
                    <td>
	                    <!-- <select id="bannerStatus" name="bannerStaus">
	                    	<option value="-1">全部</option>
...
$(function(){
	...
	function updateActivity(row){
		$.ajax({
			url: '${rc.contextPath}/banner/updateOrder',
			type:"POST",
	...
	function displayIt(id,code){
    	$.messager.confirm("提示信息","确定修改展现状态吗？",function(re){
        	if(re){
            	$.messager.progress();
            	$.post("${rc.contextPath}/banner/updateDisplayCode",{id:id,code:code},function(data){
	...
	$(function(){
		$('#s_data').datagrid({
            ...
            collapsible:true,
            idField:'id',
            url:'${rc.contextPath}/banner/jsonBannerWindowInfo',
            loadMsg:'正在装载数据...',
            fitColumns:true,
            remoteSort: true,
            singleSelect:true,
            ...
            toolbar:[{
                ...
                iconCls:'icon-add',
                handler:function(){
                    window.location.href = "${rc.contextPath}/banner/addBanner"
                }
            }],
            pagination:true,
            rownumbers:true
