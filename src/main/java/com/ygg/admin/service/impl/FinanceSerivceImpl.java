...
public class FinanceSerivceImpl implements FinanceSerivce
{
    ...
    //                "卖家备注", "发货时间", "物流公司", "物流单号", "用户类型", "下单用户名", "运费结算状态", "运费结算时间", "运费金额", "退款ID", "退款类型", "退款数量", "退款金额", "商家退款金额", "心动慈露退款金额", "退款状态", "是否已打款", "是否已收货", "结算状态",
    //                "结算时间", "承担方", "承担金额"};
    //            maxRowNum++;
        ...
        try
        {
            ...
            String[] title =
                {"订单渠道", "订单类型", "订单编号", "合并订单号", "订单状态", "结算状态", "结算时间", "付款渠道", "创建时间", "付款日期", "发货时间", "商家", "发货地", "收货人", "身份证号码", "收货地址", "省", "市", "区", "详细地址", "联系电话",
                    ...
                    "客服备注 ", "卖家备注", "发货时间", "物流公司", "物流单号", "用户类型", "下单用户名", "运费结算状态", "运费结算时间", "运费金额", "退款ID", "退款类型", "退款数量", "退款金额", "商家退款金额", "心动慈露退款金额", "退款状态", "是否已打款",
                    "是否已收货", "结算状态", "结算时间", "承担方", "承担金额", "是否超时", "罚款金额", "罚款状态", "罚款时间", "应付商家", "买家实付", "佣金", "核对", "心动慈露佣金", "N元任选优惠"};
    ...
    //                "总实付毛利", "客服备注 ", "卖家备注", "发货时间", "物流公司", "物流单号", "用户类型", "下单用户名", "运费结算状态", "运费结算时间", "运费金额", "退款ID", "退款类型", "退款数量", "退款金额", "商家退款金额", "心动慈露退款金额", "退款状态", "是否已打款",
        ...
        throws Exception
    {
        ...
        double totalWithdrawCash = oldv.getTotalWithdrawCash(); // 心动慈露优惠金额
        ...
        if (salesPrice > 0 && productCount > 0)
        {
            ...
            if (totalPrice != 0)
            {
                ...
                double totalWithdrawCashProportion = Double.parseDouble(MathUtil.round((selectPrice / totalPrice) * totalWithdrawCash, 5));// 分摊心动慈露优惠金额
        ...
        throws Exception
    {
        ...
        double allTotalGegejiaMoney = 0;// 总心动慈露分摊
        for (Map.Entry<String, Map<String, List<Map<String, Object>>>> entryBySellerId : refundMapBySellerId.entrySet())
        {
            String totalRealSellerName = "";// 真实商家名称
            double totalRefundPrice = 0;// 总退款金额
            double totalSellerMoney = 0;// 总商家分摊
            double totalGegejiaMoney = 0;// 总心动慈露分摊
            Map<String, List<Map<String, Object>>> value = entryBySellerId.getValue();
            for (Map.Entry<String, List<Map<String, Object>>> entryByOderId : value.entrySet())
            {
                List<Map<String, Object>> crRefundList = entryByOderId.getValue();
