...
";
            ...
            throws Exception
    {
         ...
         * 名称 收款账号 退款账号 链接地址
         *
         * 11 心动慈露微信 【心动慈露全球美食】 1266647901 当订单支付为微信，app渠道为19，展示为 心动慈露全球美食
         * https://pay.weixin.qq.com/index.php/refundapply?autoWXOrderNum=1002670088201510101148915431
         *
         * 12 心动慈露微信 【心动慈露全球零食汇】 1270667101 当订单支付为微信，app渠道为20，展示为 心动慈露全球零食汇
            ...
            throws Exception
    {
                ...
                && CommonUtil.isMobile(se.getFhContactMobile()))
        {
            try
            {
                ...
                sendPara.put("content", se.getRealSellerName() + "，因用户主动申请退款，您未导出发货的订单（" + order.getNumber() + "，" + fullName + "）中的部分或全部商品已被心动慈露客服同意退款，请您留意该订单的实际发货情况，防止多发或漏发，谢谢：）");
        ...
        else if (payChannel == OrderEnum.PAY_CHANNEL.ALIPAY.getCode())
        {
            ...
            if (orderType == OrderEnum.ORDER_TYPE.GEGEJIA.getCode())
            {
                if (version == 0)
                {
                    ...
                    {
                        targetFinancialAffairsCardId = 14; // 心动慈露国际支付宝
                    }
                }
                else if (version >= 1.8)
                {
                    if(realPrice <= 0.1f)
                    {
                        targetFinancialAffairsCardId = 13; // 心动慈露国内支付宝
                    }
                    else
                    {
                        targetFinancialAffairsCardId = 14; // 心动慈露国际支付宝
